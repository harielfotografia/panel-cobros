...
import { Search, Globe, FileText, User, CheckCircle2, AlertTriangle, Mail } from "lucide-react";
...
// Modo "buscar por dominio/RUT/nombre": nunca otorga sesión directa, solo confirma la
// cuenta y envía un enlace de acceso al correo ya registrado (ver /api/portal/buscar-cliente).
function BuscarForm() {
  ...
  async function handleBuscar(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setEmailEnmascarado(data.emailEnmascarado ?? "");
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {/* Confirmación — nunca se entra directo, siempre se envía un enlace al correo real */}
      <div className="px-6 pb-6 space-y-3">
        <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-sm text-blue-800 flex items-start gap-2">
          ...
          <span>
            Te enviamos un enlace de acceso a <strong>{emailEnmascarado}</strong>. Ábrelo desde tu
            correo para entrar a tu portal.
          </span>
        </div>
        {devLink && (
          <a
            href={devLink}
            className="block w-full text-center bg-gray-900 hover:bg-black text-white rounded-xl py-2.5 text-xs font-medium transition-colors"
          >
            Modo desarrollo (sin SMTP): abrir enlace directo
          </a>
        )}
        <button
...
// Modo "ingresar con email": el flujo ya seguro y probado, reutiliza /api/portal/magic-link
...
function EmailForm() {
  ...
  async function handleEnviar(e: React.FormEvent) {
    ...
    const data = await res.json();
    setMensaje(data.mensaje ?? "Si el correo está registrado, te enviamos un enlace de acceso.");
    setDevLink(data.devLink);
    setLoading(false);
  }
  ...
  return (
    <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm space-y-5">
      <form onSubmit={handleEnviar} className="space-y-3">
        ...
        {mensaje && (
          <p className="text-blue-700 text-sm bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
            {mensaje}
          </p>
        )}
        {devLink && (
        ...
        <button
          ...
          className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-xl py-3 text-sm font-semibold transition-colors flex items-center justify-center gap-2"
        >
          {loading ? "Enviando..." : <><Mail size={15} /> Enviar enlace de acceso</>}
        </button>
      </form>

      <p className="text-xs text-gray-400 text-center">
        ¿No recuerdas tu email?{" "}
        <button onClick={() => router.push("/portal/login")} className="text-blue-600 hover:underline">
          Buscar por dominio o nombre
        </button>
      </p>
    </div>
  );
...
function LoginForm() {
  ...
  const modoEmail = params.get("modo") === "email";
  ...
  return (
    <div className="w-full max-w-md">
      ...
      {modoEmail ? <EmailForm /> : <BuscarForm />}
    </div>
  );
}
