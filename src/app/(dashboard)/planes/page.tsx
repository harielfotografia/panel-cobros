...
export default function PlanesPage() {
  ...
  if (loading) return <p className="text-gray-500">Cargando planes...</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-gray-900 rounded-xl p-5 border border-gray-800 space-y-3">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={resetForm} className="bg-gray-800 hover:bg-gray-700 rounded-lg px-4 py-2 text-sm transition-colors">Cancelar</button>
          ...
          <tbody>
            {planes.map((p) => (
              <tr key={p.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                ...
                <td className="px-4 py-3 text-right text-gray-400">{p._count?.clientes ?? 0}</td>
                <td className="px-4 py-3 text-right">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${p.activo ? "bg-green-900/50 text-green-400" : "bg-gray-800 text-gray-500"}`}>
                    {p.activo ? "Activo" : "Inactivo"}
                  </span>
                </td>
                <td className="px-4 py-3 text-right space-x-2">
                  <button onClick={() => startEdit(p)} className="text-gray-400 hover:text-white text-xs">Editar</button>
                  {p.activo && <button onClick={() => handleDelete(p.id)} className="text-gray-400 hover:text-red-400 text-xs">Desactivar</button>}
                </td>
              </tr>
            ))}
            {planes.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No hay planes creados</td></tr>
            )}
          </tbody>
        </table>
      </div>
